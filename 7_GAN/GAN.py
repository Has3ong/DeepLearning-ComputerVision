...
def plotLoss(epoch):
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(__BASE_DIR__ + '/result/dcgan_loss_epoch_%d.png' % epoch)
...
def plotGeneratedImages(epoch, examples=100, dim=(10, 10), figsize=(10, 10)):
    ...
    for i in range(generatedImages.shape[0]):
        ...
        plt.axis('off')
    plt.tight_layout()
    plt.savefig(__BASE_DIR__ + '/result/dcgan_generated_image_epoch_%d.png' % epoch)

# Save the generator and discriminator networks (and weights) for later use
def saveModels(epoch):
    generator.save(__BASE_DIR__ + '/models/dcgan_generator_epoch_%d.h5' % epoch)
    discriminator.save(__BASE_DIR__ + '/models/dcgan_discriminator_epoch_%d.h5' % epoch)
